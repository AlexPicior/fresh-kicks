...
export const getStaticProps = async ({params:{slug}}) => {
  const getProductRes = await fetch(`fresh-kicks.up.railway.app/api/products/${slug}`);
  const product = await getProductRes.json();

  const getSimilarProductsRes = await fetch(`fresh-kicks.up.railway.app/api/products/${product.slug}/${product.brand}`);
