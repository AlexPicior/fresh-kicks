...
export const getStaticPaths = async () => {
  let products = [];
  fetch(`https://fresh-kicks-be-production.up.railway.app/api/products`)
...
export const getStaticProps = async ({params:{slug}}) => {
  const getProductRes = await fetch(`https://fresh-kicks-be-production.up.railway.app/api/products/${slug}`);
  const product = await getProductRes.json();

  const getSimilarProductsRes = await fetch(`https://fresh-kicks-be-production.up.railway.app/api/products/${product.slug}/${product.brand}`);
