...
export const getStaticPaths = async () => {
  let products = [];
  fetch(`fresh-kicks-be-production.up.railway.app/api/products`)
...
export const getStaticProps = async ({params:{slug}}) => {
  const getProductRes = await fetch(`fresh-kicks-be-production.up.railway.app/api/products/${slug}`);
  const product = await getProductRes.json();

  const getSimilarProductsRes = await fetch(`fresh-kicks-be-production.up.railway.app/api/products/${product.slug}/${product.brand}`);
