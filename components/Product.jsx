...
const Product = ({product}) => {
  ...
  return (
    <div className='xs:w-full lg:w-[15vw] h-fit z-10 relative mr-0'>
      ...
      {isInFavsItems(product) ? <button className='z-20 relative bottom-6 xs:left-[78%] ss:left-[83%] lg:left-[88%] xxl:left-[90%]' onClick={() => onRemoveFavs(product)}><AiFillHeart className=' h-6 w-6 max-w-fit text-rose-600'/></button> : <button className='z-20 relative bottom-6 xs:left-[78%] ss:left-[83%] lg:left-[88%] xxl:left-[90%]' onClick={() => onAddFavs(product)}><AiOutlineHeart className='h-6 w-6 max-w-fit'/></button>}
