...
const SignInComponent = () => {
  ...
  const sendCredentials = () =>{
    setErrMessage("");
    fetch('fresh-kicks.up.railway.app/api/login',{
        method: "POST",
        headers: {
    ...
    .then(response => {
      if(response.url)
      {
        if (response.url == "fresh-kicks.up.railway.app/sign_in") setErrMessage("Email or password incorrect");
        else if(response.url == "fresh-kicks.up.railway.app/") 
        {
          router.push({
            pathname: '/',
          });
