...
const SignInComponent = () => {
  ...
  useEffect(()=>{
      ...
      fetch(`https://fresh-kicks-be-production.up.railway.app/api/isAuth`)
      .then(response => {
  ...
  const sendCredentials = () =>{
    setErrMessage("");
    fetch('https://fresh-kicks-be-production.up.railway.app/api/login',{
        method: "POST",
        headers: {
    ...
    .then(response => {
      if(response.url)
      {
        if (response.url == "https://fresh-kicks-be-production.up.railway.app/sign_in") setErrMessage("Email or password incorrect");
        else if(response.url == "https://fresh-kicks-be-production.up.railway.app/") 
        {
          router.push({
            pathname: '/',
          });
